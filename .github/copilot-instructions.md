...
### ⚠️ **CRITICAL: Microservice Separation**

**PramaIA is organized as INDEPENDENT MICROSERVICES, each in its own Git remote:**

| Folder | Remote Name | Purpose |
|--------|-------------|---------|
| `PramaIAServer/` | `PramaIA` | Backend API + React frontend |
| `PramaIA-PDK/` | `PramaIA-PDK` | Plugin Development Kit & workflow engine |
| `PramaIA-LogService/` | `PramaIA-LogService` | Centralized logging microservice |
| `PramaIA-VectorstoreService/` | `PramaIA-VectorstoreService` | Vector storage with ChromaDB |
| `PramaIA-Reconciliation/` | `PramaIA-Reconciliation` | Data sync & integrity service |
| `PramaIA-Agents/` | `PramaIA-Agents` | Autonomous agents (document monitoring) |
| **Root files** | `PramaIA-Commons` | Configuration, documentation, shared scripts (`.env`, `start-all.ps1`, `docs/`, etc.) |

### 🚨 **REMEMBER:**
- Each microservice folder has its OWN remote repository
- Changes to `PramaIAServer/` go to `PramaIA` remote
- Changes to `PramaIA-LogService/` go to `PramaIA-LogService` remote
- Changes to root-level files (configuration, docs, start scripts) go to `PramaIA-Commons` remote
- **NEVER commit microservice folders into `PramaIA-Commons`**
- **NEVER mix microservice code with commons code**

### Commit Strategy
```bash
# For root/commons changes
git add <root-level-files>
git commit -m "..."
git push origin main  # PramaIA-Commons

# For microservice changes (example: LogService)
cd PramaIA-LogService
git add <files>
git commit -m "..."
git push origin main  # PramaIA-LogService remote
```
